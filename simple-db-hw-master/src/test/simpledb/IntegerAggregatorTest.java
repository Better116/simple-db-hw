...
//     verify how the results progress after a few merges
  ...
  @Test public void mergeSumNoGrouping() throws Exception {
    scan1.open();
    IntegerAggregator agg = new IntegerAggregator(-1, Type.INT_TYPE, 1, Aggregator.Op.SUM);

    for (int[] step : sumNoGrouping) {
      agg.mergeTupleIntoGroup(scan1.next());
      OpIterator it = agg.iterator();
      it.open();
